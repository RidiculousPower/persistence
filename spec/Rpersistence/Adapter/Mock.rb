...
class Rpersistence::Adapter::Mock
  ...
  ########################################
	...
	def get_bucket_key_class_for_object_id( persistence_id )

    return @bucket_key_class_for_id[ persistence_id ]
  ...
  def put_object!( object )
    ...
    object_persistence_hash = object.persistence_hash_to_port
		...
		complex_properties.each do |this_complex_property|
			@complex_property_for_id[ object.persistence_id ][ this_complex_property ] 	= true
			@delete_cascades_for_id[ object.persistence_id ][ this_complex_property ]		= object.delete_cascades?
  ...
  def get_object( persistence_id, bucket )
		@buckets[ bucket ] ||= Hash.new
		@complex_property_for_id[ persistence_id ] ||= Hash.new
		persistence_hash_from_port	=	@buckets[ bucket ][ persistence_id ]
		if persistence_hash_from_port
			persistence_hash_from_port.each do |this_property, this_value|
				if @complex_property_for_id[ persistence_id ][ this_property ]
				  bucket, key, klass = get_bucket_key_class_for_object_id( persistence_id )
				  this_property  = [ klass, get_object( this_value, bucket ) ]
				end
			end    
		end
  ...
  def delete_object!( persistence_id, bucket )
		@buckets[ bucket ] ||= Hash.new
		persistence_hash_from_port	=	@buckets[ bucket ][ persistence_id ]
		persistence_hash_from_port.each do |this_property, this_value|
      if @complex_property_for_id[ persistence_id ][ this_property ] and @delete_cascades_for_id[ persistence_id ][ this_property ]
        bucket, key, klass = get_bucket_key_class_for_object_id( persistence_id )
        delete_object!( this_value )
      end
		end
		@buckets[ bucket ].delete( persistence_id )
    # and now delete the object's ID reference
    @id_for_bucket_key.delete( persistence_id )
    return self
  end
